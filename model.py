...
class DGCNN(nn.Module):
    def __init__(self, emb_dims=512):
        ...
        self.conv4 = nn.Sequential(nn.Conv2d(128, 256, kernel_size=1, bias=False),
                                   nn.BatchNorm2d(256),
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv5 = nn.Sequential(nn.Conv2d(256, 512, kernel_size=1, bias=False),
                                   nn.BatchNorm2d(512),
                                   nn.LeakyReLU(negative_slope=0.2))
        # self.conv6 = nn.Sequential(nn.Conv1d(1024, emb_dims, kernel_size=1, bias=False),
        self.conv6 = nn.Sequential(nn.Conv1d(1536, emb_dims, kernel_size=1, bias=False),
    ...
    def forward(self, x):
        ...
        global_feat = x5.mean(dim=-1, keepdim=True).repeat(1, 1, N)

        # x = torch.cat((x1, x2, x3, x4, x5), dim=1)
        # x = self.conv6(x).view(B, -1, N)
        x = torch.cat((x1, x2, x3, x4, x5, global_feat), dim=1)
        x = self.conv6(x).view(B, -1, N)
...
class DGCNNClassification(nn.Module):
    def __init__(self, emb_dims, output_channels, feat_dim=0):
        ...
        self.conv1 = nn.Sequential(nn.Conv2d(6 + feat_dim, 64, kernel_size=1, bias=False),
                                   self.bn1,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv2 = nn.Sequential(nn.Conv2d(64, 64, kernel_size=1, bias=False),
                                   self.bn2,
    ...
    def forward(self, x):
        ...
        if self.feat_dim != 0:
            x_feat = x[:, 3:, :].unsqueeze(-1).repeat(1, 1, 1, 20)  # k = 20
            x = torch.cat([x_k, x_feat], dim=1)
        else:
            x = x_k
